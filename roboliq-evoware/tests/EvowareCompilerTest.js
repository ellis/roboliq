...
describe('EvowareCompiler', function() {
	describe('compileStep', function () {
		...
		it("should compile transporter._moveLidFromContainerToSite", function() {
			...
			it("should compile transporter._moveLidFromSiteToContainer", function() {
				const table = {};
				const protocol = _.merge({}, protocol0, {
					objects: {
						plate1Lid: {
				...
				should.deepEqual(results, [
					[
						{
							"line": "Transfer_Rack(\"1\",\"1\",0,1,0,0,0,\"1\",\"96-Well Plate\",\"Narrow\",\"\",\"\",\"Some Carrier\",\"Some Carrier\",\"Some Carrier\",\"1\",\"2\",\"1\");",
							"effects": {
								"plate1.location": "site1",
								"plate1Lid.location": "plate1",
								"EVOWARE.romaIndexPrev": 0
							},
							"tableEffects": [
								[ [ "Some Carrier", 1, 1 ], { "label": "site1", "labwareModelName": "96-Well Plate" } ],
		...
		it("should handle suspending at system.runtimeLoadVariables", () => {
			const table = {};
			const protocol = _.merge({}, protocol0, {
				roboliq: "v1",
				steps: {
			...
			const agents = ["robot1"];
			const options = {timing: false, variables: {ROBOLIQ: "AAA", SCRIPTDIR: "C:\\Here", SCRIPTFILE: "C:\\Here\\myscript.out.json"}};
			const results = EvowareCompiler.compileStep(table, protocol, agents, [], undefined, {}, [], options);
			// console.log("results: "+JSON.stringify(results, null, '\t'));
			should.deepEqual(results, [
				[
					{ "line": "Execute(\"AAA evowareRuntimeLoadVariables C:\\Here\\myscript.out.json 1\",2,\"\",2);" },
					{ "line": "StartScript(\"C:\\Here\\continue.esc\");", "file": { "filename": "continue.esc", "data": "" } }
				]
			]);
		});

		it("should handle resuming at system.runtimeLoadVariables", () => {
			const table = {};
			const protocol = _.merge({}, protocol0, {
				roboliq: "v1",
				steps: {
	...
	describe("compile", () => {
		it("should open browser with info about the script", () => {
			const table = {};
			const protocol = _.merge({}, protocol0, {
				roboliq: "v1",
				steps: {
		...
		it("should call `initRun` script", () => {
			...
			const [{lines}] = EvowareCompiler.compile(table, protocol, agents, options);
			// console.log({lines});
			should(lines).containEql('Execute("AAA initRun C:\\Here\\myscript.out.json",2,"",2);');
		});
