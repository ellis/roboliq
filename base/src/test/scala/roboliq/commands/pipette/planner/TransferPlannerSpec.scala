	...
	describe("TransferPlanner") {
		describe("BSSE configuration") {
			...
			it("should work for 1 item") {
				...
				val item_l = List(
					TransferPlanner.Item(List(vss_P1_A01), vss_P1_B01, LiquidVolume.ul(50))
				)
			...
			it("should work for 2 neighboring items") {
				...
				val item_l = List(
					TransferPlanner.Item(List(vss_P1_A01), vss_P1_C01, LiquidVolume.ul(50)),
					TransferPlanner.Item(List(vss_P1_B01), vss_P1_D01, LiquidVolume.ul(50))
				)
			...
			it("should work for 2 non-neighboring items") {
				...
				val vss_P1_D01 = checkObj(p.getObjFromDbAt[VesselSituatedState]("P1(D01)", List(0)))
				val item_l = List(
					TransferPlanner.Item(List(vss_P1_A01), vss_P1_D01, LiquidVolume.ul(50)),
					TransferPlanner.Item(List(vss_P1_B01), vss_P1_C01, LiquidVolume.ul(50))
				)
