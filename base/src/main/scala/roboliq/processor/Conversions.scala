...
object Conversions {
	private val logger = Logger("roboliq.processor.Conversions")
	...
	def tipStateToJson(tipState: TipState): JsValue = {
		...
		JsObject(Map(
			...
			"srcsEntered" -> RqResult.toResultOfList(srcsEntered.toList.map(D.toJson[Liquid])).map(JsArray(_)),
			"destsEntered" -> RqResult.toResultOfList(destsEntered.toList.map(D.toJson[Liquid])).map(JsArray(_)),
