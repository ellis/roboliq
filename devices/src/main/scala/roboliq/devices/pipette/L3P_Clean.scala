						...
						val twvpsAD = cmd.tips.toSeq.map(tip => {
							...
							(policyA_?, policyD_?) match {
								case (Some(policyA), Some(policyD)) =>
									val twvpA = new L2A_AspirateItem(tip, well1A, wellStateA.liquid, nVolume, policyA)
									val twvpD = new L2A_DispenseItem(tip, tipState.liquid, well1D, wellStateD.liquid, nVolume, policyD)
									(twvpA, twvpD)
								case _ =>
