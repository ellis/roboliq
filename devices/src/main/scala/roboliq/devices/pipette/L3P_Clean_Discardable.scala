...
class L3P_Clean_Discardable(robot: PipetteDevice, locationDrop: String) extends CommandCompilerL3 {
	...
	def compile(ctx: CompilerContextL3, cmd: CmdType): CompileResult = {
		cmd.degree match {
			...
			case _ =>
				CompileTranslation(cmd, Seq(
						L2C_TipsDrop(cmd.tips, locationDrop),
						L2C_Wash(cmd.tips, locationDrop),
						L2C_TipsGet(cmd.tips, locationDrop),
						))
