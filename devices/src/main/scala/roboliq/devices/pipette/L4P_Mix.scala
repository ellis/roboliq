...
class L4P_Mix extends CommandCompilerL4 {
	type CmdType = L4C_Mix
	...
	def translate(states: RobotState, cmd: CmdType): Either[String, Command] = {
		val destConfs = new ArrayBuffer[WellConfigL2]
		val bAllOk = cmd.dests.forall(dest => {
			val confs = PipetteHelperL4.getWells1(states, dest)
			if (confs.isEmpty) {
				false
			}
			else {
		...
		if (bAllOk) {
			Right(L3C_Mix(new L3A_MixArgs(destConfs.toSet, cmd.mixSpec)))
		}
		else {
			Left("missing well")
		}
