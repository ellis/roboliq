...
a lot of power to handle complex experiments.  The `data` property understands the
following parameters:

* `source`: this is the name of a `Design` or `Variable` that contains an array of data rows.
* `where`: this is a boolean [mathjs](http://mathjs.org/) expression that is evaluated for each data row -- only rows for which the expression evaluates to true are activated.
* `orderBy`: an array of column names for ordering rows.
