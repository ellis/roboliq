 ...
 * var p = makePlanner(basicExample,llpl,utils);
 * p.ppPlan(p.plan());
 *
...
function makePlanner(spec,llpl,utils) {
  ...
  function refinePlan(plan) {
	...
	bindingsArray = llpl.conjoin(utils.copyIt(providers[j]["actionOrMethod"][providerType]["preconditions"]),
				     [freshBindings]);
  ...
  function quitToInspect(plan) {
    if ( planIsASuccess(plan) || ( plan["depth"] == 60 ) ) { return(true) }
    else { return(false); }
  }
  ...
  return({taskInstances: taskInstances,
