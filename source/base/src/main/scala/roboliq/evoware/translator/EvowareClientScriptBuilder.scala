...
class EvowareClientScriptBuilder(agentName: String, config: EvowareConfig) extends ClientScriptBuilder(agentName) {
	...
	private def readerRun(
		...
		for {
			...
			carrierE <- getAgentObject[Carrier](deviceIdent, s"missing evoware carrier for device `${deviceIdent}`")
			l <- {
				if (carrierE.partNo_? == Some("Tecan part no. 30016056 or 30029757") || cmd.device.label == Some("Infinite M200")) {
					readerRun_InfiniteM200(identToAgentObject_m, cmd, carrierE)
				}
				else {
					Context.error(s"Driver missing for reader device `${cmd.device.label}`")
				}
			}
		} yield l
	}
	...
	private def readerRun_InfiniteM200(
		...
		for {
			...
			// Token
			val token = L0C_Facts(carrierE.sName, carrierE.sName+"_Measure", cmd.outputFilename + "|" + programData)
			// Return
