 ...
 *
 * Sample usage:
 * var utils = makeUtilities();
 * var llpl = makeInterpreter(utils);
 * llpl.initializeDatabase(sicpDB);
 *
 * This assumes that sicpDB is a global variable in which an array of
 * rules and assertions have been defined.  See the sample database 
 * in the file sicpDB.js
 * This also assumes that the file utilities.js has been loaded into
 * the browser.  It contains a set of necessary auxillary functions.
 *
...
function makeInterpreter(utils) {
  ...
  function eeval(toEvaluate,bindingsArray) {
    ...
    var resultantBindingsArray = [];
    for ( var op in expression ) { operator = op; }
  ...
  function evalExpression(expression,bindings) {
	...
	return(iexpression);
      ...
      var operator, i;
      for ( var op in iexpression ) { operator = op; }
      var argValues = [];
