...
class WashTipsCmdSpec extends CommandSpecBase {
	ignore("pipette.low.washTips") {
		describe("BSSE configuration") {
			implicit val p = makeProcessorBsse(
				...
				JsonParser("""{
					"cmd": [
					  { "cmd": "pipette.low.washTips", "items": [{"tip": "TIP1", "well": "P1(A01)", "volume": "50ul", "policy": { "id": "Wet", "pos": "WetContact" }}] }
					]
					}""").asJsObject
			)
