...
object Printer {
	...
	def parseWellId(id: String): RsResult[Tuple4[String, String, Int, Int]] = {
		id match {
			case WellColRowPattern(idPlate, sRow, sCol) => RsSuccess((idPlate, sRow+sCol, (sRow.charAt(0) - 'A'), sCol.toInt - 1))
			case WellRowPattern(idPlate, sRow) => RsSuccess((idPlate, sRow, sRow.toInt - 1, 0))
			case WellNullPattern(idPlate) => RsSuccess((idPlate, "", 0, 0))
			case _ => RsSuccess((id, "", 0, 0))
		}
	}
}
