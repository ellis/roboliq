...
\startitemize
\item Level 4 AST: encapsulates the abstract protocol and the robot configuration, but it may be incomplete due to missing parameters.
...
\item Level 1 AST: contains only a small, basic instruction set, appropriate for translation into the language of the target robot.
...
We distinguish between two types of commands: 1) the {\em Basic Command Set} is a small finite set of commands which the Robot Plugin will need to translate into its own format, and 2) an extensible higher-level set of commands which can make use of both basic commands as well as other commands from the higher-level set.
